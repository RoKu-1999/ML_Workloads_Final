...
df = df.groupby(['function', 'sgx', 'percent_train'])['percent_spent'].sum().reset_index()
...
pivot_df = df.pivot_table(index='percent_train', columns=['sgx', 'function'], values='percent_spent', aggfunc='sum').fillna(0)
...
for idx, sgx_value in enumerate([0, 1]):
    ...
    if sgx_value == 0:
        axes[idx].set_title(f"Training without Intel SGX", fontsize=34)
    else:
        axes[idx].set_title(f"Training inside Intel SGX", fontsize=34)
        
    axes[idx].set_ylabel("Time spent in Functions in Percent", fontsize=30)
...
axes[0].set_xlabel("Training Data Percent", fontsize=30)
axes[1].set_xlabel("Training Data Percent", fontsize=30)
...
plt.savefig("dt_3_cpu_functions_stacked.pdf")
