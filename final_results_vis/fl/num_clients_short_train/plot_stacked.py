...
df = df.groupby(['function', 'sgx', 'num_clients'])['percent'].sum().reset_index()

print(df)
# Filter the data to only include rows where percent_spent is at least 0.01
# df = df[df['percent'] >= 0.01]

# Prepare the data for stacked bar chart
pivot_df = df.pivot_table(index='num_clients', columns=['sgx', 'function'], values='num_clients', aggfunc='sum').fillna(0)
...
sorted_common_functions = df[df['function'].isin(common_functions)].groupby('function')['num_clients'].sum().sort_values(ascending=False).index.tolist()
...
for idx, sgx_value in enumerate([1, 0]):
    # Plot with color map
    ...
    if sgx_value == 0:
        axes[idx].set_title(f"Training without Intel SGX", fontsize=20)
    else:
        axes[idx].set_title(f"Training inside Intel SGX", fontsize=20)
        
    axes[idx].set_ylabel("Time spent in Functions in Percent", fontsize=18)
    
    # Adjust tick label sizes
    axes[idx].tick_params(axis='both', labelsize=12)
    axes[idx].tick_params(labelrotation=0)

# Set x-label for both subplots with adjusted size
axes[0].set_xlabel("Training Data Percent", fontsize=18)
axes[1].set_xlabel("Training Data Percent", fontsize=18)

# Add a shared legend outside of the plots
fig.legend([plt.Rectangle((0,0),1,1, fc=color_map[func]) for func in reversed(sorted_functions)], list(reversed(sorted_functions)), loc='center right', fontsize=14)

fig.subplots_adjust(right=0.7, wspace=0.3)  # Adjust spacing between subplots and right boundary to account for legend

plt.savefig("cpu_functions_stacked.png")
