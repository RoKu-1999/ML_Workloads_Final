...
plt.figure()
# Plotting
train = sns.barplot(data=df, x='trees', y='train_time', hue='sgx')
train.set_xlabel('Trees')
train.set_ylabel('Train Time (seconds)')
train.set_title('Train Time vs. Trees by SGX')

label_overheads(train)

plt.savefig('rf_train_num_trees.png')

plt.figure()

test = sns.barplot(data=df, x='trees', y='test_time', hue='sgx')
test.set_xlabel('Trees')
test.set_ylabel('Test Time (seconds)')
test.set_title('Test Time vs. Trees by SGX')

label_overheads(test)

plt.savefig('rf_test_num_trees.png')
